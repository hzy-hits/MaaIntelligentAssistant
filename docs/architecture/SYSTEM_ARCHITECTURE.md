...
MAA 智能控制中间层是一个基于 Rust 的智能游戏控制系统，通过 Function Calling 协议让大模型直接控制 MaaAssistantArknights。系统采用异步任务队列架构，提供16个完整的MAA Function Calling工具，支持自然语言交互和智能游戏自动化。

## 核心设计理念

### 1. 并发安全优先
通过异步任务队列将HTTP请求与MAA执行分离
- **无锁设计**: 消息传递替代共享状态锁
...
src/
├── bin/
│   └── maa-intelligent-server.rs    # 唯一服务器入口
├── maa_core/                        # MAA 核心模块  
│   ├── mod.rs                       # 核心类型定义和回调处理
...
### 重构后
- **成果**: 清晰的模块结构，异步队列架构，16个完整工具
- **收益**: 架构简化，开发效率提升，功能完善

## 设计决策记录 (ADR)

### ADR-001: 选择异步队列 vs 共享锁
